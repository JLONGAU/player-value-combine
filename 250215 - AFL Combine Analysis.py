...
# --- Fix dtype and NA issues before merging ---

# Ensure both key columns exist in both dataframes
for key in ['Player_ID', 'Year']:
    if key not in combine_df.columns:
        raise KeyError(f"{key} missing from combine_df")
    if key not in draft_df.columns:
        raise KeyError(f"{key} missing from draft_df")

# Convert 'Year' to consistent nullable integer type
combine_df['Year'] = pd.to_numeric(combine_df['Year'], errors='coerce').astype('Int64')
draft_df['Year'] = pd.to_numeric(draft_df['Year'], errors='coerce').astype('Int64')

# Convert 'Player_ID' to string and fill NAs with empty string
combine_df['Player_ID'] = combine_df['Player_ID'].astype('string').fillna('')
draft_df['Player_ID'] = draft_df['Player_ID'].astype('string').fillna('')

# --- End Fix dtype and NA issues before merging ---
...
# --- normalize Year on both dataframes before merging ---
# draft_success_df is created a few lines earlier; force its dtype too
combine_df['Year'] = pd.to_numeric(combine_df['Year'], errors='coerce').astype('Int64')
draft_success_df['Year'] = pd.to_numeric(draft_success_df['Year'], errors='coerce').astype('Int64')

# Option A (recommended): keep rows with NA Year; they just won't match
combine_df = combine_df.merge(draft_success_df, on='Year', how='left')

# Option B (if you do not want rows with missing Year at all)
# combine_df = combine_df.dropna(subset=['Year']).merge(draft_success_df, on='Year', how='left')
# ---  ENd normalize Year on both dataframes before merging ---
...
combine_df = combine_df.merge(
    ...
    on='Year',
    how='left',
    suffixes=('', '_rhs')  # keep LHS names; RHS same-name columns get _rhs
)

# Use the RHS value (the threshold from draft_success_df)
combine_df['Is_Draft_Success_Matches'] = (
    (combine_df['Total_Matches'] >= combine_df['Draft_Success_Matches_rhs']).astype(int)
)
